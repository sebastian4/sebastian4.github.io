function AudioSynthView() {
	...
	// __audioSynth.setSampleRate(10000);
	__audioSynth.setSampleRate(100);
	...
	var fnPlayNote = function(note, octave) {
		var output = $("#output").text()+' '+note;
		console.log('playNote '+note+' '+octave);
		$("#output").text(output);
		$("#console").text('note:'+note);
		if (!window.playingSong) {
			console.log("caching")
			window.pcache.push([ note+','+(octave-4) , 4 ]);
	...
	var fnPlayKeyboard = function(e) {
		...
		console.log(e);
		// $("#console").text('playKey '+e.keyCode);
	...
	var fnPlaySong = function(arr) {
		...
		console.log(arr);
		window.playingSong = true;

		if(arr.length>0) {
		
			var noteLen = 2000*(1/parseInt(arr[0][1]));
	...
	Object.defineProperty(this, 'playSong', {
		value: fnPlaySong
	});
	
	Object.defineProperty(this, 'playKey', {
