...
var Level = Base.extend({
	...
	nextLoad: function() {
		...
		this.reset();
		this.load(definedLevels[this.id]);
...
//
var Stone_a = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 957, 676);
	},
}, 'stone_a');
var Stone_n = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 993, 676);
	},
}, 'stone_n');
var Stone_d = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1028, 676);
	},
}, 'stone_d');
var Stone_o = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062, 676);
	},
}, 'stone_o');
var Stone_l = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1098, 676);
	},
}, 'stone_l');
var Stone_t = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1134, 676);
	},
}, 'stone_t');
var Stone_m = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 958, 713);
	},
}, 'stone_m');
var Stone_i = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 994, 713);
	},
}, 'stone_i');
var Stone_s = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1029, 712);
	},
}, 'stone_s');
var Stone_e = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1065, 712);
	},
}, 'stone_e');
var Stone_b = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1099, 713);
	},
}, 'stone_b');
var Stone_y = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1135, 713);
	},
}, 'stone_y');
var Stone_u = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 957, 749);
	},
}, 'stone_u');
var Stone_p = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 994, 749);
	},
}, 'stone_p');
var Stone_z = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1029, 749);
	},
}, 'stone_z');
var Stone_v = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1065, 748);
	},
}, 'stone_v');
var Stone_f = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1099, 748);
	},
}, 'stone_f');
var Stone_r = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1135, 748);
	},
}, 'stone_r');
var Stone_w = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1135, 784);
	},
}, 'stone_w');
//
var Wood_sebdolphin1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 792 +0);
	},
}, 'wood_sebdolphin1');
var Wood_sebdolphin2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 792 +0);
	},
}, 'wood_sebdolphin2');
var Wood_sebdolphin3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 792 +32);
	},
}, 'wood_sebdolphin3');
var Wood_sebdolphin4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 792 +32);
	},
}, 'wood_sebdolphin4');
var Wood_missa1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 792 +0);
	},
}, 'wood_missa1');
var Wood_missa2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 792 +0);
	},
}, 'wood_missa2');
var Wood_missa3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 792 +32);
	},
}, 'wood_missa3');
var Wood_missa4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 792 +32);
	},
}, 'wood_missa4');
var Wood_sebnmissa1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 864 +0);
	},
}, 'wood_sebnmissa1');
var Wood_sebnmissa2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 864 +0);
	},
}, 'wood_sebnmissa2');
var Wood_sebnmissa3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 864 +32);
	},
}, 'wood_sebnmissa3');
var Wood_sebnmissa4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 864 +32);
	},
}, 'wood_sebnmissa4');
var Wood_sebnmissb1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 864 +0);
	},
}, 'wood_sebnmissb1');
var Wood_sebnmissb2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 864 +0);
	},
}, 'wood_sebnmissb2');
var Wood_sebnmissb3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 864 +32);
	},
}, 'wood_sebnmissb3');
var Wood_sebnmissb4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 864 +32);
	},
}, 'wood_sebnmissb4');
var Wood_sebnmissc1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 935 +0);
	},
}, 'wood_sebnmissc1');
var Wood_sebnmissc2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 935 +0);
	},
}, 'wood_sebnmissc2');
var Wood_sebnmissc3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 935 +32);
	},
}, 'wood_sebnmissc3');
var Wood_sebnmissc4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 935 +32);
	},
}, 'wood_sebnmissc4');
var Wood_missb1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 935 +0);
	},
}, 'wood_missb1');
var Wood_missb2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 935 +0);
	},
}, 'wood_missb2');
var Wood_missb3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 935 +32);
	},
}, 'wood_missb3');
var Wood_missb4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 935 +32);
	},
}, 'wood_missb4');
var Wood_landlove1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 1005 +0);
	},
}, 'wood_landlove1');
var Wood_landlove2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 1005 +0);
	},
}, 'wood_landlove2');
var Wood_landlove3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +0, 1005 +32);
	},
}, 'wood_landlove3');
var Wood_landlove4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1062 +32, 1005 +32);
	},
}, 'wood_landlove4');
var Wood_sebnland1 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 1005 +0);
	},
}, 'wood_sebnland1');
var Wood_sebnland2 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 1005 +0);
	},
}, 'wood_sebnland2');
var Wood_sebnland3 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +0, 1005 +32);
	},
}, 'wood_sebnland3');
var Wood_sebnland4 = Ground.extend({
	init: function(x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 1128 +32, 1005 +32);
	},
}, 'wood_sebnland4');
...
var Mario = Hero.extend({
	init: function(x, y, level) {
		console.log("mario level ="+level.id);
		if (level.id==14) {
			console.log("last level");
			ended = true;
		}
		//mylevel = level;
		console.log("rem posx="+posx);
		if (posx>0) {
			console.log("changing x to "+posx);
			x = posx;
			posx = 0;
		}
		if (posy>0) {
			console.log("changing y to "+posy);
			y = posy;
			posy = 0;
		}
		this.standSprites = [
...
$(document).ready(function() {
	...
	var lvid = getUrlVars()["lvid"];
	posx = getUrlVars()["posx"];
	posy = getUrlVars()["posy"];
	sprs = getUrlVars()["sprs"];
	gift = getUrlVars()["gift"];
	tabt = getUrlVars()["tabt"];
	ended = false;
	
	if (isNaN(lvid)) { lvid = 0; 
	} else { 
	 if (lvid>200)	{ lvid = Number(lvid-200); }
	 else if (lvid<8)	{ lvid = Number(lvid); }
	 else { lvid = 0; }
	}
	if (sprs==null) { sprs = "landon"; }
	if (tabt==null) { tabt = 'no'; };
	if (isNaN(posx)) { posx = 0; } else { posx = Number(posx); }
	if (isNaN(posy)) { posy = 0; } else { posy = Number(posy); }
	if (gift==null) { gift = 'yes'; }
	
	console.log("lvid="+lvid);
	console.log("posx="+posx);
	console.log("posy="+posy);
	console.log("sprs="+sprs);
	console.log("gift="+gift);
	console.log("tabt="+tabt);
	
	if (sprs=='landon') { images.sprites=images.sprites_lan; 
	$("#live").css("background-image","url(Content/mario-sprites.land.png)"); }
	else if (sprs=='mario') { images.sprites=images.sprites_mar; 
	}
	else { images.sprites=images.sprites_lan;
	$("#live").css("background-image","url(Content/mario-sprites.land.png)"); }
	if (gift=='no') { 
	$('#photoframe').remove(); console.log("remove gift"); }
	
	if (tabt == 'no') {
		$('#buttons').remove();
	} else { // tabt==yes
		$('body').css("background-image","none");
		$("#btn_up").bind( "touchstart", function(e){
			keys.up = true;
		});
		$("#btn_up").bind( "touchend", function(e){
			keys.up = false;
		});
		$("#btn_down").bind( "touchstart", function(e){
			keys.down = true;
		});
		$("#btn_down").bind( "touchend", function(e){
			keys.down = false;
		});
		...
		$("#btn_ext").bind( "touchstart", function(e){
			keys.accelerate = true;
		});
		$("#btn_ext").bind( "touchend", function(e){
			keys.accelerate = false;
		});
	}
	
	var level = new Level('world');
	level.load(definedLevels[lvid]);
	level.start();
	keys.bind();
